...
public class FileDirUtil {
     ...
     * @param copyPathTo
     */
    public static void copyAll(String copyPathFrom,String copyPathTo) {
        try  {
            (new  File(copyPathTo)).mkdirs();  //如果文件夹不存在  则建立新文件夹
            File  a=new  File(copyPathFrom);
            String[]  file=a.list();
            File  temp=null;
            for  (int  i  =  0;  i  <  file.length;  i++)  {
                if(copyPathFrom.endsWith(File.separator)){
                    temp=new  File(copyPathFrom+file[i]);
                }
                else{
                    temp=new  File(copyPathFrom+File.separator+file[i]);
                }

                if(temp.isFile()){
                    FileInputStream  input  =  new  FileInputStream(temp);
                    FileOutputStream  output  =  new  FileOutputStream(copyPathTo  +  "/"  +
                ...
                if(temp.isDirectory()){//如果是子文件夹
                    copyAll(copyPathFrom+"/"+file[i],copyPathTo+"/"+file[i]);
                }
            }
        }
        catch  (Exception  e)  {
